...
const transformField = (astField, astParent) => {
  ...
  return interpolatify`
    ${camelcasify(astParent.internalKey)}.${camelcasify(astField.internalKey)} = _${camelcasify(astParent.internalKey)}.field('${escapeSingleQuotes(astField.key)}').${astField.valueRequired ? 'required' : 'optional'}StringValue();
  `;
};
  ...
  return interpolatify`
    {
      ...
      ${astFieldset.entries.map(astEntry => interpolatify`
        ${camelcasify(astParent.internalKey)}.${camelcasify(astFieldset.internalKey)}.${camelcasify(astEntry.internalKey)} = _${camelcasify(astFieldset.internalKey)}.entry('${escapeSingleQuotes(astEntry.key)}').${astEntry.valueRequired ? 'required' : 'optional'}StringValue();
      `).join('\n')}
    }
  `;
};
...
const transformList = (astList, astParent) => {
  return interpolatify`
    ${camelcasify(astParent.internalKey)}.${camelcasify(astList.internalKey)} = _${camelcasify(astParent.internalKey)}.${astList.required ? 'required' : 'optional'}List('${escapeSingleQuotes(astList.key)}').${astList.valuesRequired ? 'required' : 'optional'}StringValues();
  `;
};
