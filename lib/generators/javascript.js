...
const transformEmpty = (astEmpty, astParent) => {
  ...
  return interpolatify`
    ${camelcasify(astParent.internalKey)}.${camelcasify(astEmpty.internalKey)} = _${camelcasify(astParent.internalKey)}.optionalEmpty('${escapeSingleQuotes(astEmpty.key)}') !== null;
  `;
};

const transformField = (astField, astParent) => {
  // TODO: Missing differentiation between required element and required value, also elsewhere
  return interpolatify`
    ${camelcasify(astParent.internalKey)}.${camelcasify(astField.internalKey)} = _${camelcasify(astParent.internalKey)}.field('${escapeSingleQuotes(astField.key)}').${camelcasify(`${astField.valueRequired ? 'required' : 'optional'}_${astField.valueTypes[0].name}`)}Value();
  `;
};
...
const transformFieldset = (astFieldset, astParent) => {
  return interpolatify`
    {
      const _${camelcasify(astFieldset.internalKey)} = _${camelcasify(astParent.internalKey)}.${astFieldset.required ? 'required' : 'optional'}Fieldset('${escapeSingleQuotes(astFieldset.key)}');
      ${camelcasify(astParent.internalKey)}.${camelcasify(astFieldset.internalKey)} = {};
      ${astFieldset.entries.map(astEntry => interpolatify`
        ${camelcasify(astParent.internalKey)}.${camelcasify(astFieldset.internalKey)}.${camelcasify(astEntry.internalKey)} = _${camelcasify(astFieldset.internalKey)}.entry('${escapeSingleQuotes(astEntry.key)}').${camelcasify(`${astEntry.valueRequired ? 'required' : 'optional'}_${astEntry.valueTypes[0].name}`)}Value();
      `).join('\n')}
    }
  `;
};
...
const transformList = (astList, astParent) => {
  return interpolatify`
    ${camelcasify(astParent.internalKey)}.${camelcasify(astList.internalKey)} = _${camelcasify(astParent.internalKey)}.${astList.required ? 'required' : 'optional'}List('${escapeSingleQuotes(astList.key)}').${camelcasify(`${astList.valuesRequired ? 'required' : 'optional'}_${astList.valueTypes[0].name}`)}Values();
  `;
};
    ...
    return interpolatify`
      {
        ...
        const _${camelcasify(astSection.internalKey)} = _${camelcasify(astParent.internalKey)}.${astParent.required ? 'required' : 'optional'}Section('${escapeSingleQuotes(astSection.key)}');
