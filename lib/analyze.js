...
const PRIORITIZED_TYPES = [
  { name: 'boolean', loader: enotype.boolean },
  { name: 'integer', loader: enotype.integer },
  { name: 'float', loader: enotype.float },
  { name: 'date', loader: enotype.date },
  { name: 'datetime', loader: enotype.datetime },
  { name: 'url', loader: enotype.url },
  { name: 'email', loader: enotype.email },
  { name: 'ipv4', loader: enotype.ipv4 },
  { name: 'color', loader: enotype.color },
  { name: 'lat_lng', loader: enotype.latLng },
  { name: 'json', loader: enotype.json }
];
...
const inferTypes = value => {
  ...
  if(value !== null) {
    for(const type of PRIORITIZED_TYPES) {
      try {
        type.loader(value);
        prioritizedCompatibleTypes.push(type.name);
      } catch(error) {
  ...
  prioritizedCompatibleTypes.push('string');
